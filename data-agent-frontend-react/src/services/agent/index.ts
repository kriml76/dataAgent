 ...
 */
export interface AgentApiKeyApiResult extends ApiResponse {
...
class AgentService {
  ...
  async list(status?: string, keyword?: string): Promise<Agent[]> {
    ...
    return apiClient.get<Agent[]>(`${API_BASE_URL}/list`, { params });
  }
  ...
  async get(id: number): Promise<Agent | null> {
    try {
      return await apiClient.get<Agent>(`${API_BASE_URL}/${id}`);
    } catch (error: any) {
  ...
  async create(agent: Omit<Agent, 'id'>): Promise<Agent> {
    ...
    return apiClient.post<Agent>(API_BASE_URL, agentData);
  }
  ...
  async update(id: number, agent: Partial<Agent>): Promise<Agent | null> {
    try {
      ...
      return await apiClient.put<Agent>(`${API_BASE_URL}/${id}`, agentData);
    } catch (error: any) {
      if (error.response?.status === 404) {
        return null;
      }
  ...
  async publish(id: number): Promise<Agent | null> {
    try {
      return await apiClient.post<Agent>(`${API_BASE_URL}/${id}/publish`);
    } catch (error: any) {
      if (error.response?.status === 404) {
        return null;
      }
  ...
  async offline(id: number): Promise<Agent | null> {
    try {
      return await apiClient.post<Agent>(`${API_BASE_URL}/${id}/offline`);
    } catch (error: any) {
      if (error.response?.status === 404) {
        return null;
      }
  ...
  async getApiKey(id: number): Promise<AgentApiKeyResponse | null> {
    try {
      const response = await apiClient.get<AgentApiKeyApiResult>(
        `${API_BASE_URL}/${id}/api-key`
      );
      if (response.success && response.data) {
        return response.data;
      }
      throw new Error(response.message || '获取 API Key 失败');
    } catch (error: any) {
      if (error.response?.status === 404) {
        return null;
      }
  ...
  async deleteApiKey(id: number): Promise<AgentApiKeyResponse> {
    const response = await apiClient.delete<AgentApiKeyApiResult>(
      `${API_BASE_URL}/${id}/api-key`
    );
    if (response.success && response.data) {
      return response.data;
