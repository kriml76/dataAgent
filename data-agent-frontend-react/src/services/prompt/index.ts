...
class PromptService {
  ...
  async listByType(promptType: string, agentId?: string | number): Promise<PromptConfig[]> {
    ...
    const response = await apiClient.get<PromptConfigResponse>(
  ...
  async save(config: PromptConfig): Promise<PromptConfigResponse> {
    return await apiClient.post<PromptConfigResponse>(`${API_BASE_URL}/save`, config);
  }
  ...
  async enable(id: number): Promise<PromptConfigResponse> {
    return await apiClient.post<PromptConfigResponse>(`${API_BASE_URL}/${id}/enable`);
  }
  ...
  async disable(id: number): Promise<PromptConfigResponse> {
    return await apiClient.post<PromptConfigResponse>(`${API_BASE_URL}/${id}/disable`);
  }
  ...
  async delete(id: number): Promise<PromptConfigResponse> {
    return await apiClient.delete<PromptConfigResponse>(`${API_BASE_URL}/${id}`);
  }
  ...
  async batchEnable(ids: number[]): Promise<PromptConfigResponse> {
    return await apiClient.post<PromptConfigResponse>(`${API_BASE_URL}/batch-enable`, ids);
  }
  ...
  async batchDisable(ids: number[]): Promise<PromptConfigResponse> {
    return await apiClient.post<PromptConfigResponse>(`${API_BASE_URL}/batch-disable`, ids);
  }
  ...
  async updatePriority(id: number, priority: number): Promise<PromptConfigResponse> {
    return await apiClient.post<PromptConfigResponse>(`${API_BASE_URL}/${id}/priority`, { priority });
  }
}
